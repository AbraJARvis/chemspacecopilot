...
QSAR_TRAINING_INSTRUCTIONS = [
    ...
    "  - When the user explicitly asks for LightGBM, first prepare or reuse a tabular QSAR dataset, then call the dedicated LightGBM tools (`describe_lightgbm_backend`, `describe_lightgbm_environment`, `train_lightgbm_model`) rather than routing the request through Chemprop or TabICL.",
    ...
    "  - If the user explicitly asks for Morgan only or RDKit only, follow that request exactly and do not add the other representation.",
    "  - On lighter local compute profiles (`local_light`, `local_standard`), use `descriptor_set=basic` by default, so the standard combined representation becomes Morgan + RDKit basic.",
    "  - On heavier GPU-capable profiles (`heavy_validation`), use `descriptor_set=all` by default, so the standard combined representation becomes Morgan + RDKit all.",
...
QSAR_REPORT_INSTRUCTIONS = [
    ...
    "  - When the training handoff contains an `activity_cliff_feedback` block, add a dedicated `Activity cliffs` subsection inside `Partie 2 : Entrainement et validation`.",
    "  - In that subsection, explicitly report: `mode`, `index_name`, `flagged_count`, counts `none / low / medium / high`, and the `tiering_policy` as observed in the handoff.",
    "  - When `mode=annotate_only`, state explicitly that no compounds were removed and that OOF residuals are contextual only and were not used to assign tiers.",
    "  - When feedback loops were requested, add one compact loop table with `variant_id`, `removed_tiers`, `removed_count`, and `remaining_rows`, plus the recommended variant when present.",
    "  - When `activity_cliff_feedback` includes artifact paths such as `annotated_training_csv`, `summary_path`, or plot files, mention the main ones briefly in the `Activity cliffs` subsection and list them compactly in `Partie 4 : Fichiers generes`.",
...
AGENT_TEAM_INSTRUCTIONS = [
    ...
    "  - Activity-cliff annotation is available by default during standard QSAR training when the dataset supports it:",
    "    • Default mode is `annotate_only`: compute SALI-based annotation, tiers `none/low/medium/high`, and keep the full training set",
    "    • The activity-cliff index is `SALI` in V2",
    "  - Feedback loops are available only when the user requests them explicitly during QSAR training:",
    "    • '1 feedback loop' means baseline + one retrain without the `high` tier",
    "    • '2 feedback loops' means baseline + `drop_high` + `drop_high_medium`",
    "    • '3 feedback loops' means baseline + `drop_high` + `drop_high_medium` + `drop_high_medium_low`",
    "    • Never request or invent more than 3 loops",
    "    • Do not override advanced activity-cliff parameters unless the user explicitly asks for them; keep the defaults (`index=sali`, `k_neighbors=10`, `oof_folds=5`)",
    "    • Tiering is derived from SALI only; OOF residuals may be discussed for interpretation but are not used to assign `low/medium/high`",
    "    • This activity-cliff loop is supported for standard QSAR training only, not for benchmark mode in V2",
