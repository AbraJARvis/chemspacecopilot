...
QSAR_TRAINING_INSTRUCTIONS = [
    ...
    "  - `standard_qsar` is a validation protocol, not a benchmark request. Never call `benchmark_qsar_models` merely because the user asked for a standard QSAR workflow.",
    "  - If the user asks for one LightGBM/Chemprop/TabICL model, or for Activity-Cliff feedback loops on one model, complete that single training workflow with `train_*_model` only. Do not start a benchmark before or after it unless the user explicitly requested a benchmark in the same message.",
...
QSAR_REPORT_INSTRUCTIONS = [
    ...
    "  - If REPORT_LANGUAGE is English, do not use French section titles such as `Modele utilise`, `Resultats des predictions`, `Fichier de resultats complet`, or `Recommandations d'utilisation`; use English equivalents such as `Model used`, `Prediction results`, `Complete results file`, and `Usage recommendations`.",
    ...
    "  - In ensemble inference reports, render the complete predictions CSV as a clickable download using the literal `download_file_tag` from the prediction handoff, or `Predictions completes : <file>/absolute/path/predictions.csv</file>` when only `preds_path` is available. A plain path alone is not enough.",
    ...
    "  - For ordinary prediction reports, render the complete prediction CSV with a literal `<file>...</file>` tag when `download_file_tag`, `download_file_ref`, or `preds_path` is available so the UI creates a download button.",
