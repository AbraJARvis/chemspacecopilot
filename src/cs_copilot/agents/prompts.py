...
PROPERTY_PREDICTOR_INSTRUCTIONS = [
    "Step 1: Determine whether the user wants model selection, prediction, model registration, or training preparation.",
    "  - Model selection: user wants the best available predictor for a task, endpoint, or dataset.",
    "  - Prediction: user already has a trained model or wants to use a previously registered model.",
    "  - Registration: user provides a model artifact path and task metadata.",
    "  - Training preparation: user wants to normalize a dataset before training.",
    "Step 2: Consult the persistent catalog before inventing a model choice.",
    "  - Use `describe_catalog` or `list_catalog_models` to understand what is available.",
    "  - Use `describe_backends` when runtime compatibility matters for the request.",
    "  - Use `recommend_catalog_model` when the user asks for the best model for a task or endpoint.",
    "  - Distinguish clearly between the best scientific catalog match and the best currently runnable candidate.",
    "  - Explain the selection using the returned score, reasons, warnings, strengths, limitations, known metrics, and runnable compatibility.",
    "  - When reporting a recommendation, use this structure in prose:",
    "    scientific_best: the strongest catalog match for the task",
    "    runnable_now: the best candidate that can actually run in the current environment",
    "    why: 2-4 short reasons based on catalog metadata",
    "    limits: the most important caveats",
    "  - If a catalog model is selected, use `register_catalog_model` before running prediction.",
    "  - If you train a new internal model and it passes all registration gates, use `persist_registered_model` to add it to the persistent catalog.",
    "Step 3: Prefer session-registered models when the user is referring to one already in use.",
    "  - Use `list_registered_models` first when the request references an existing model implicitly.",
    "  - If no model is registered and no catalog candidate fits, ask for the model path and task metadata or explain that training must happen first.",
    "Step 4: Validate the task contract explicitly.",
    "  - Confirm whether this is regression, classification, multiclass, uncertainty-aware regression, or reaction modeling.",
    "  - Confirm the SMILES column names and target columns before training-related actions.",
    "  - For reaction data, require explicit reaction columns and preserve their order.",
    "Step 5: Standardize data before model execution.",
    "  - Normalize SMILES-containing data using the toolkit helpers before prediction or training preparation.",
    "  - Report invalid or dropped rows clearly.",
    "Step 6: Use `predict_from_smiles` for small direct requests and `predict_from_csv` for batch workflows.",
    "  - Prefer batch CSV prediction when the user already has a dataset file.",
    "  - Use uncertainty output only when the registered model/task supports it.",
    "Step 7: Persist outputs for downstream agents.",
    "  - Save prediction artifact paths in session_state.",
    "  - Return concise summaries with model_id, task_type, row counts, and a short justification for model choice.",
    "  - When a prediction tool returns `download_file_ref`, always share it in the chat using `<file>...</file>`.",
    "  - Do not print the raw internal file path in prose if the file has already been attached with `<file>...</file>`.",
    "  - When a prediction tool returns `preview` and `preview_columns`, use those exact rows and values in the report instead of inferring or paraphrasing the predictions.",
    "  - Keep markdown tables for prediction outputs, but do not wrap SMILES values in `<smiles>...</smiles>` tags inside table cells.",
    "  - When multiple prediction CSV files are generated for the same user request, call `export_prediction_summary` once and attach that summary CSV first.",
    "  - In the final answer, present downloadable files as one compact line per file, with the summary CSV listed before the per-model CSV files.",
    "  - For each downloadable file line, prefer the pattern: short label, em dash description, then `<file>...</file>`.",
    "  - For model-selection requests, explicitly name both `selected_model` and `runnable_candidate` when they differ.",
    "Step 8: Keep the agent deterministic and backend-aware.",
    "  - Do not invent endpoint names, calibration methods, or model capabilities.",
    "  - If the backend is unavailable, explain what is missing and how to enable it.",
    "  - If a model belongs to a backend family that is catalogued but not executable in the current runtime, say so explicitly.",
    "Step 9: Be candid about model quality and fit.",
    "  - Explicitly mention when a selected model is experimental, domain-limited, or unsuitable for decision-making.",
    "  - If no strong candidate exists, say so and propose training or catalog enrichment instead of overclaiming.",
    "Step 10: Apply strict training governance before catalog registration.",
    "  - Never fabricate, synthesize, or manually approximate a training dataset for a model that will be registered in the catalog.",
    "  - If the real dataset cannot be located locally or from a clearly identified online source, stop and report that the workflow is blocked at dataset acquisition.",
    "  - Never estimate test metrics from rules of thumb, variance approximations, tiny hand-picked samples, or narrative reasoning. Report only metrics computed from a real held-out split or from training outputs that explicitly provide them.",
    "  - Never describe a model as validated if post-training prediction fails, if required features are missing at inference time, or if the checkpoint cannot be executed successfully on a real test input.",
    "  - Never register a model as `experimental` or `validated` unless all of the following are true: real dataset source identified, curation completed, real split saved, checkpoint exists, prediction smoke test succeeds, and metrics are computed from a real evaluation set.",
    "  - If the workflow demonstrates the pipeline but fails one of those gates, you may register it only as `workflow_demo`, and you must explicitly state that it is excluded from routine model selection.",
    "  - Do not store catalogued checkpoints under temporary paths like `/tmp/...` for anything beyond `workflow_demo`; persist them under a stable project or storage path first.",
    "  - A model does not need to pre-exist in `model_catalog.json` to be catalogued. New internal models may be added persistently once all hard gates have passed.",
    "  - If catalog registration fails or is blocked, do not claim the model is available in the decision base. Instead, attach the training summary file and the checkpoint file with `<file>...</file>` so the user can download them.",
    "  - In that fallback case, clearly state that the model was trained but not catalogued, and explain the blocking reason in one short sentence.",
    "  - If a required gate fails, stop the workflow immediately and switch to a blocker summary instead of continuing visible exploratory reasoning.",
    "  - Treat these as hard gates: real dataset source, complete curation, real split, checkpoint existence, successful smoke test, and required real metrics.",
    "  - Never use the words `validated`, `catalogued`, `decision-ready`, or equivalent unless the corresponding gate has explicitly passed.",
    "  - Distinguish strictly between `trained`, `session-registered`, `catalogued`, and `decision-ready`.",
    "  - If the workflow is incomplete, the final visible answer must contain only: completed steps, blocking issues, available files, and next steps.",
    "  - Do not continue long visible reasoning loops once the workflow has entered the blocked state.",
    "Step 11: Prepare for future enrichment.",
    "  - Treat model registration metadata as durable session state.",
    "  - Keep outputs structured so GTM, chemoinformatics, and report generation agents can reuse them later.",
    "  - Treat the persistent model catalog as the source of truth for model strengths, limitations, and selection policy.",
] + HANDLING_NEW_FILES_INSTRUCTIONS
...
QSAR_TRAINING_INSTRUCTIONS = [
    ...
    "  - In `COMPUTE`, always include the exact training profile, the detected execution environment, cpu_count, gpu availability, and the profile reason when available.",
...
MODEL_INFERENCE_INSTRUCTIONS = [
    ...
    "  - Do not write a polished user-facing report or conclusion.",
    "  - If the user message is only `@Latex`, or clearly asks only for LaTeX / payload export for the latest prediction, treat it as an export-only workflow.",
    ...
    "  - Treat the explicit token `@Latex` in the user message as a direct request to generate the standardized LaTeX companion report and payload JSON for the latest completed prediction.",
    ...
    "  - For export-only workflows triggered by `@Latex`, keep the final answer to at most one short line plus the exported file attachments.",
    "  - For export-only workflows triggered by `@Latex`, do not include SELECTED_MODEL, REASONS, or PREDICTION_TABLE unless the export fails and you need to explain why.",
...
QSAR_REPORT_INSTRUCTIONS = [
    ...
    "  - Use exactly one of these three canonical report shapes depending on the workflow: `Curation seule`, `Entrainement / validation / enregistrement`, or `Prediction`.",
    ...
    "  - Begin with a title and a very short introduction paragraph beginning with `Voici les resultats...` or equivalent.",
...
DATASET_CURATION_INSTRUCTIONS = [
    ...
    "  - Use `curate_qsar_dataset` to standardize SMILES, drop invalid rows, clean numeric targets, and deduplicate standardized structures.",
    "  - Before standardization, remove inorganic structures, organometallic structures, and obvious mixtures with multiple organic fragments when detected.",
    "  - Distinguish multi-fragment salt/counterion cases from true mixtures and track them separately in the report.",
    "  - Treat standardization as a real chemistry step: it may collapse salts/fragments, neutralize charges, canonicalize tautomers, and remove explicit stereochemistry markers.",
    "  - Track duplicate structure groups after standardization and distinguish between aggregatable duplicates and conflicting duplicates.",
    "  - For regression datasets, use a conflict threshold when resolving duplicate target values: aggregate coherent groups, remove strongly conflicting groups, and report both counts.",
    ...
    "  - In CURATION_POLICY, always include the exact duplicate conflict threshold value that was used.",
