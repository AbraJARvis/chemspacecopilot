...
QSAR_REPORT_INSTRUCTIONS = [
    ...
    "  - For summaries of an already-existing ensemble, do not describe the workflow as a new creation. Use `Resume de l'ensemble` / `Ensemble Summary`, then report existing metadata, components, evaluations, artifacts, and limitations.",
    ...
    "  - Never wrap `<file>...</file>` tags in backticks, code fences, block quotes, or markdown table cells.",
    ...
    "  - For existing ensemble summary reports, list JSON/Markdown artifacts such as `metadata.json`, `ensemble.json`, `selection_evidence.json`, and `selection_report.md` as plain paths or inline code paths. Do not create one download button per minor artifact unless the user explicitly asks to download them.",
