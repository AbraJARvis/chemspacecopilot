...
QSAR_REPORT_INSTRUCTIONS = [
    ...
    "  - Report the neighborhood policy explicitly when present: fingerprint, radius, bit count, similarity metric, similarity threshold, `top-k neighbors = <value>` (for SALI default, `top-k neighbors = 10`), flag threshold, and normalization.",
    "  - If no loops were requested, state explicitly that no compounds were removed.",
    "  - If loops were requested, include a compact variants table with variant_id, removed_tiers, removed_count, remaining_rows, training_completed, and comparable fixed-holdout metrics when present.",
    "  - For loop variants, distinguish dataset-level removed_count from split-level removed_from_train_count/effective_train_count; validation and test holdouts remain non-filtered.",
