...
QSAR_TRAINING_INSTRUCTIONS = [
    ...
    "  - Never create a QSAR ensemble merely because a benchmark was requested. Set `create_ensemble=true` only when the user explicitly asks for an ensemble, consensus, vote, model aggregation, or combination of the best models.",
    "  - QSAR ensemble creation from benchmark is V1-limited to `benchmark_robust_qsar`. If the user requests an ensemble with `fast_local`, `standard_qsar`, or `challenging_qsar`, explain that robust_qsar is required for ensemble construction.",
